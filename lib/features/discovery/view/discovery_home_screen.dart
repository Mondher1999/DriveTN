...
class DiscoveryHomeScreen extends StatelessWidget {
        ...
        child: CustomScrollView(
          slivers: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        if (result != null && context.mounted) {
                          ...
                          context
                              .read<CarsCubit>()
                              .setSearchDates((result.start, result.end));
            ...
            _sectionTitle('Expériences',
                top: 24,
                bottom: 16,
                subtitle: 'Choisissez votre expérience de conduite'),
            SliverToBoxAdapter(
              child: SizedBox(
                height: 120,
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    GestureDetector(
                      onTap: () =>
                          _onVoirToutTap(context, topPicksFilter: true),
                      child: Text(
                        'Voir tout',
                        style: AppTypography.body(
                            size: 13,
            ...
            _sectionTitle('Agences de confiance',
                top: 20,
                bottom: 10,
                subtitle: 'Partenaires vérifiés et bien notés'),
            SliverToBoxAdapter(
              child: SizedBox(
                height: 120,
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      ...
                      child: _AgencyCard(
                        agency: agency,
                        onTap: () => _onAgencyTap(context, agency.id),
                      ),
                    );
                  },
                ),
  ...
  Widget _sectionTitle(String text,
      {double top = 28, double bottom = 6, String? subtitle}) {
  ...
  Future<void> _onExperienceTap(
      BuildContext context, Set<CarCategory> categories) async {
    HapticFeedback.lightImpact();
    final result = await LocationDateFlowSheet.show(
      context,
      initialStep: 0,
  ...
  Future<void> _onVoirToutTap(
    ...
    if (result != null && context.mounted) {
      ...
      if (offersFilter) {
        context
            .read<CarsCubit>()
            .applyFilters(priceRange: const RangeValues(0, 150));
      } else if (budgetFilter) {
        context
            .read<CarsCubit>()
            .applyFilters(priceRange: const RangeValues(0, 99));
      }
      context.go('/home/explorer');
    }
  }

  Future<void> _onAgencyTap(BuildContext context, String agencyId) async {
    HapticFeedback.lightImpact();
    final result = await LocationDateFlowSheet.show(
      context,
      initialStep: 0,
    ...
    if (result != null && context.mounted) {
      ...
      context.read<CarsCubit>().setSearchLocation(result.location);
      context.read<CarsCubit>().setSearchDates((result.start, result.end));
      context.read<CarsCubit>().filterByAgency(agencyId);
      context.go('/home/explorer');
    }
  }
}
